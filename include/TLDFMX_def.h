 ...
 *
 ******************************************************************************/
#ifndef INCL_TLDFMX_DEF_H_
#define INCL_TLDFMX_DEF_H_
...
#if defined _WIN32
    #if defined(_MSC_VER)
        #if defined(TLDFMX_EXPORT)
            #define TLDFMX_API __declspec(dllexport)
        #else
            #define TLDFMX_API __declspec(dllimport)
        #endif
    #else
        #if defined(TLDFMX_EXPORT)
            #define TLDFMX_API __attribute__ ((dllexport))
        #else
            #define TLDFMX_API __attribute__ ((dllimport))
        #endif
    #endif
    #define TLDFMX_INTERNAL /**/
#else
    #if __GNUC__ >= 4
        #define TLDFMX_API __attribute__ ((visibility ("default")))
        #define TLDFMX_INTERNAL  __attribute__ ((visibility ("hidden")))
    #else
        #define TLDFMX_API
        #define TLDFMX_INTERNAL
    #endif
